...
module.exports = {
  ...
  async execute(client, interaction, data, args) {
    ...
    let fieldsLinks = {
      name: '**»** Bağlantılar',
      value: `**•** [Destek Sunucusu](https://discord.gg/vp4XCbrHg5) • [Davet Bağlantısı](${client.settings.invite})`,
      inline: false
    };
    ...
    embedModeration.fields.push({
      name: "**»** Bilgi ⚠️",
      value:
        `**•** [Cynleax+'i sunucuna eklemek için buraya tıkla!](https://top.gg/bot//)`,
    });
    ...
    let embedMessageFilters = {
      color: client.settings.embedColors.default,
      author: {
        name: `${client.user.username} • Güvenlik Sistemleri`,
        icon_url: client.settings.icon,
      },
    ...
    let embedNC = {
      color: client.settings.embedColors.default,
      author: {
        name: `${client.user.username} • Cynleax+ Coin`,
        icon_url: client.settings.icon,
      },
      title: `Bir komut hakkında bilgi almak için \`/komutlar <Komut>\` yazabilirsiniz.`,
      description:
    ...
    const embedMainPage = {
      ...
      description:
        ...
        `🔰 • Güvenlik Sistemleri (**${embedMessageFilters.fields.length - 1}**)\n` +
        ...
        `Cynleax+ hergün gelişen bir bottur eksik özellikler yakında ayarlanacaktır.\n` +
        `Bot hakkında şikayet ve öneriler için Discord: rushlw.`
        /*`${(data.user.readDateOfChanges < client.settings.updateDate) ?
          `✉️ Okunmamış yenilikler mevcut! \`/yenilikler\` yazarak okuyabilirsic!` :
      ...
      .addComponents(
        new Discord.StringSelectMenuBuilder()
          ...
          .addOptions([
            ...
            {
              label: 'Cynleax+ Coin Komutları',
              emoji: '💰',
              //description: 'CynleaxCoin ile ilgili tüm komutlar.',
              value: 'NCOption',
            },
            {
              label: 'Bot Komutları',
              emoji: '🤖',
              description: 'Cynleax ile ilgili komutlar.',
              value: 'botOption',
            },
          ])
      );
